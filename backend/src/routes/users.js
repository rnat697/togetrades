...
router.post('/register', async(req,res)=>{
    ...
    if(!username || !email || !password) return res.sendStatus(422).send('All fields are compulsory');

    // No duplicate usernames or emails
    let existingUsername = await User.findOne({username});
    let existingEmail = await User.findOne({email});
    if (existingUsername || existingEmail) return res.sendStatus(409).send('User already exists with this email or username');
